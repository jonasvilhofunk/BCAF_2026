...
class AdaptiveHSIPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans, embed_dim, spectral_group_size=None, norm_layer=nn.LayerNorm):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.spectral_size > 1:
            ...
            x = torch.stack(output_groups, dim=1)
...
class HSIRGBAdapterSwin(SwinTransformer):
    """HSI backbone that uses a learnable adapter to convert HSI channels to 3 RGB-like channels."""
    def __init__(
        self,
        original_in_chans,
        img_size,
...
def build_swin_transformer(config=None):
    """Factory to build Swin (RGB/HSI) backbones."""
