"""UNet-style segmentation decoder heads for RGB (UNetHead) and HSI (HSIUNetHead) modalities,
plus a head factory (build_segmentation_head) and a fused-feature decoder (FusedFeatureFusionHead)."""
...
def build_segmentation_head(config, feature_dim):
    ...
    if config['modality'] == 'hsi':
        return HSIUNetHead(in_dims, head_cfg)
