...
class FocalLoss(nn.Module):
    """
    Focal Loss built on top of per-pixel CrossEntropy (no ignore_index).
    alpha can be:
      - None
      - scalar
      - tensor/list/ndarray of shape (C,) for per-class weights
    """
    def __init__(self, alpha=None, gamma=2.0, reduction='mean'):
        super().__init__()
        if isinstance(alpha, (list, np.ndarray)):
            alpha = torch.tensor(alpha, dtype=torch.float32)
        self.alpha = alpha  # tensor or scalar or None
        self.gamma = gamma
...
class SegmentationLoss(nn.Module):
    """
    Composite loss:
      - 'ce', 'dice', 'ce_dice', 'focal', 'focal_dice'
    class_weights can be list/ndarray/tensor; move to device before using criterion if needed.
    """
    def __init__(self, loss_type='ce_dice', num_classes=None, ce_weight=1.0, dice_weight=1.0, focal_gamma=2.0, class_weights=None):
        super().__init__()
    ...
    def forward(self, logits, targets):
        parts = []
        if 'focal' in self.loss_type:
            parts.append(self.ce_weight * self.focal(logits, targets))
        if 'ce' in self.loss_type and 'focal' not in self.loss_type:
...
def calculate_class_weights_from_frequencies(class_frequencies, strategy, device, epsilon=1e-6, num_classes_for_norm=None):
    ...
    if strategy == 'frequency':
        ...
        weights = 1.0 / (torch.sqrt(class_frequencies) + epsilon)
    else:
        return None
