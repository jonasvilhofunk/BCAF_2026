...
class Augmentation:
    def _parse_size_config(self, size_val, cfg_name):
        """
        Parse target size from config.
        Accepts int -> (s, s) or (h, w) as list/tuple.
        """
    ...
    def __init__(self, config, is_train=True, data_modality_to_load='rgb', hsi_stats_dict=None):
        ...
        # HSI normalization (applied after geometry)
        ...
        # PCA for HSI
        self.pca_config = self.config.get('pca_transform', {})
        self.pca_enable = self.pca_config.get('enable', False)
        # accept either 'savgol' (correct) or 'sovgol' (typo) from YAML
        self.pca_savgol_preapplied = bool(self.pca_config.get('savgol', self.pca_config.get('sovgol', False)))
        self.pca_path = self.pca_config.get('pca_path', None)

        # Savitzky-Golay settings
        self.savgol_window_length = int(self.pca_config.get('savgol_window_length', 15))
        self.savgol_polyorder = int(self.pca_config.get('savgol_polyorder', 3))
        self.savgol_derivative_order = int(self.pca_config.get('savgol_derivative', 0))

        # Precompute SavGol kernel (derivative=0 fast path)
        self._savgol_kernel = None
        if self.pca_savgol_preapplied and self.savgol_derivative_order == 0:
            if self.savgol_window_length % 2 == 0:
                self.savgol_window_length += 1
            if self.savgol_window_length < self.savgol_polyorder + 2:
                self.savgol_window_length = self.savgol_polyorder + 2
                if self.savgol_window_length % 2 == 0:
                    self.savgol_window_length += 1
            if savgol_coeffs:
                coeffs = savgol_coeffs(self.savgol_window_length, self.savgol_polyorder, deriv=0, use='conv')
                self._savgol_kernel = torch.tensor(coeffs, dtype=torch.float32)
            else:
                self._savgol_kernel = torch.full((self.savgol_window_length,), 1.0 / self.savgol_window_length, dtype=torch.float32)

        self.pca_mean = None
        self.pca_components = None
        self.pca_n_components_target = None
        self.pca_n_features_in = None
        self.pca_loaded_successfully = False

        if self.pca_enable and self.pca_path and 'hsi' in self.data_modality_to_load:
            try:
                with open(self.pca_path, 'r') as f:
                    pca_yaml_data = yaml.safe_load(f)
                p = pca_yaml_data.get('pca_parameters', {})
                self.pca_mean = torch.tensor(p.get('mean'), dtype=torch.float32)
                self.pca_components = torch.tensor(p.get('components'), dtype=torch.float32)
                self.pca_n_components_target = p.get('n_components_target')
                self.pca_n_features_in = p.get('n_features_in')

                ok = (
                    self.pca_mean is not None and
                    self.pca_components is not None and
                    self.pca_n_components_target is not None and
                    self.pca_n_features_in is not None and
                    self.pca_mean.ndim == 1 and self.pca_mean.shape[0] == self.pca_n_features_in and
                    self.pca_components.ndim == 2 and
                    self.pca_components.shape == (self.pca_n_components_target, self.pca_n_features_in)
                )
                if ok:
                    self.pca_loaded_successfully = True
                    print(f"Info (Augmentation): PCA loaded ({self.pca_n_features_in}->{self.pca_n_components_target}).")
                    if self.pca_savgol_preapplied:
                        print("Info (Augmentation): PCA expects SavGol pre-filtered spectra.")
                else:
                    print("Error (Augmentation): PCA parameter shape mismatch. Disabled.")
                    self.pca_enable = False
            except FileNotFoundError:
                print(f"Error (Augmentation): PCA file not found: {self.pca_path}. Disabled.")
                self.pca_enable = False
            except Exception as e:
                print(f"Error (Augmentation): PCA YAML load failed: {e}. Disabled.")
                self.pca_enable = False
        elif self.pca_enable and not self.pca_path and 'hsi' in self.data_modality_to_load:
            print("Warning (Augmentation): PCA enabled but 'pca_path' missing. Disabled.")
            self.pca_enable = False

        # HSI spectral padding (after normalization if no PCA)
        ...
        # Random rotate/scale/flip (train-only)
        ...
        # HSI spectral jitter (train-only)
    ...
    def _process_loaded_hsi_stats(self, stats_dict, method):
        """Validate/convert HSI stats to tensors for normalization."""
        processed = {}
        try:
            if method == "minmax":
    ...
    def _apply_normalization(self, img, img_type):
        ...
        if img_type == 'hsi':
            ...
            if self.hsi_norm_enable and self.hsi_stats_loaded_successfully:
                ...
                # Add 'minmax' branch here if needed later.
            return hsi

        return img

    def _apply_pca_transform(self, hsi_tensor):
        """Project HSI tensor using PCA on valid pixels only; keep background zero."""
        if not self.pca_enable or not self.pca_loaded_successfully or hsi_tensor is None:
            return hsi_tensor
        if not isinstance(hsi_tensor, torch.Tensor):
            raise TypeError(f"HSI data must be Tensor for PCA, got {type(hsi_tensor)}")

        C, H, W = hsi_tensor.shape
        if C != self.pca_n_features_in:
            print(f"Warning (Augmentation): HSI channels ({C}) != PCA input ({self.pca_n_features_in}). Skipping PCA.")
            return hsi_tensor

        valid_mask = (hsi_tensor != 0).any(dim=0)
        if not valid_mask.any():
            return torch.zeros(self.pca_n_components_target, H, W, device=hsi_tensor.device, dtype=hsi_tensor.dtype)

        mean = self.pca_mean.to(hsi_tensor.device)               # (C,)
        comps = self.pca_components.to(hsi_tensor.device)        # (K,C)

        data_flat = hsi_tensor.reshape(C, -1).T                  # (H*W, C)
        data_centered = data_flat - mean
        projected = torch.matmul(data_centered, comps.T)         # (H*W, K)
        hsi_pca = projected.T.reshape(self.pca_n_components_target, H, W)

        if (~valid_mask).any():
            hsi_pca[:, ~valid_mask] = 0

        return hsi_pca

    def _pad_hsi_spectral(self, hsi_tensor):
        """
        Zero-pad/truncate channels to target size. Assumes (C,H,W).
        """
        if not isinstance(hsi_tensor, torch.Tensor):
        ...
        # F.pad expects (W_left, W_right, H_left, H_right, C_left, C_right) for 3D (C,H,W)
    ...
    def _apply_single_geometric_transform(self, img, transform_name, param, is_label=False):
        ...
        fill_val = 0  # keep background zero
    ...
    def _apply_geometric_transforms_sequentially(self, rgb_input, hsi_input, label_input_pil):
        ...
        # Stage 1: HSI spectral pre-processing (no normalization here)
        if hsi_tensor is not None:
            hsi_tensor = self._apply_spectral_jitter(hsi_tensor)
            if self.pca_enable and self.pca_savgol_preapplied:
                hsi_tensor = self._apply_savgol_filter(hsi_tensor)
            if self.pca_enable and self.pca_loaded_successfully:
                C, H, W = hsi_tensor.shape
                N = self.pca_n_features_in
                if C < N:
                    pad_c = N - C
                    pad = torch.zeros(pad_c, H, W, dtype=hsi_tensor.dtype, device=hsi_tensor.device)
                    hsi_tensor = torch.cat([hsi_tensor, pad], dim=0)
                elif C > N:
                    hsi_tensor = hsi_tensor[:N]
                hsi_tensor = self._apply_pca_transform(hsi_tensor)

        # Stage 2: Spatial transforms (RGB/HSI/Label + mask)
        current_rgb_img = rgb_tensor
        current_hsi_img = hsi_tensor
        current_mask = None
        if current_hsi_img is not None:
            current_mask = (current_hsi_img != 0).any(dim=0, keepdim=True).float()  # (1,H,W)
        ...
        # Re-zero HSI background using transformed mask
        ...
        # Stage 3: Photometric + normalization
        ...
        # If PCA produced RGB-like channels, optionally apply ImageNet (rgb) normalization to PCA output
        if current_hsi_img is not None and self.pca_enable and self.pca_loaded_successfully:
            # if PCA target is 3 channels and user requested rgb_normalization, apply it
            if self.pca_n_components_target == 3 and self.rgb_norm_enable:
                current_hsi_img = self._apply_normalization(current_hsi_img, 'rgb')

        if current_hsi_img is not None and self.hsi_norm_enable:
            current_hsi_img = self._apply_normalization(current_hsi_img, 'hsi')

        if current_hsi_img is not None and self.hsi_spectral_pad_enable and self.hsi_target_spectral_channels is not None and (not self.pca_enable):
    ...
    def __call__(self, sample):
        input_image_data = sample.get('image')  # dict for rgb_hsi or Tensor
        label_input_pil = sample.get('label')   # PIL Image or None
        ...
        # Convert final label to LongTensor
    ...
    def _apply_spectral_jitter(self, hsi_tensor):
        """Brightness/contrast jitter along spectral dimension, skipping zero background."""
        ...
        valid_pixels = pixels[valid_mask]  # (N_valid, C)
    ...
    def _apply_savgol_filter(self, hsi_tensor):
        """
        Savitzky-Golay smoothing along channel axis.
        derivative=0 uses a fast torch conv kernel; derivative>0 uses scipy if available.
        """
        if hsi_tensor is None or not self.pca_savgol_preapplied:
            return hsi_tensor
        ...
        if self.savgol_derivative_order > 0:
            if savgol_coeffs is None:
                warnings.warn("scipy not available; skipping SavGol derivative smoothing.")
                return hsi_tensor
            from scipy.signal import savgol_filter
            arr = hsi_tensor.detach().cpu().numpy()
            arr_flat = arr.reshape(C, -1)
            valid_flat = valid_mask.view(-1).cpu().numpy()
            arr_valid = arr_flat[:, valid_flat]
            if arr_valid.shape[1] > 0:
                win = min(self.savgol_window_length if self.savgol_window_length <= C else (C if C % 2 == 1 else C - 1), C)
                arr_valid_sm = savgol_filter(
                    arr_valid,
                    window_length=win,
                    polyorder=min(self.savgol_polyorder, C - 1),
                    deriv=self.savgol_derivative_order,
                    axis=0,
                    mode='interp'
                )
                arr_flat[:, valid_flat] = arr_valid_sm
            smoothed = torch.from_numpy(arr_flat.reshape(C, H, W)).to(hsi_tensor.device, hsi_tensor.dtype)
            return smoothed

        # Fast conv1d path
        if self._savgol_kernel is None:
            return hsi_tensor

        k = self._savgol_kernel.to(hsi_tensor.device, hsi_tensor.dtype)
        pad = k.numel() // 2

        x_perm = hsi_tensor.permute(1, 2, 0).reshape(-1, 1, C)  # (H*W,1,C)
        valid_flat = valid_mask.view(-1)
        if valid_flat.any():
            x_valid = x_perm[valid_flat]               # (N_valid,1,C)
            x_pad = F.pad(x_valid, (pad, pad), mode='replicate')
            kernel = k.view(1, 1, -1)
            sm_valid = F.conv1d(x_pad, kernel)         # (N_valid,1,C)
            x_perm[valid_flat] = sm_valid
        smoothed = x_perm.reshape(H, W, C).permute(2, 0, 1)  # (C,H,W)
        return smoothed